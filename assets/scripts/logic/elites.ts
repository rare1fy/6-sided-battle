...
export function processEliteDice(
  ...
  if (elite && game.battleTurn % ELITE_CONFIG.eliteDiceCycle === 0) {
    ...
    result.floats.push({ text: '+碎裂骰子', color: '#f87171', target: 'player' });
  ...
  if (boss) {
    ...
    if (hpRatio < ELITE_CONFIG.bossCurseHpRatio && game.battleTurn % ELITE_CONFIG.bossCurseCycle === 0) {
      ...
      result.floats.push({ text: '+诅咒骰子', color: '#c084fc', target: 'player' });
      result.sound = 'enemy_skill';
      result.triggered = true;
      return result;
    }
...
export interface EliteArmorResult {
  ...
  log: string;
  /** 浮动文字 */
  float: { text: string; color: string; target: string; icon?: string };
  /** 音效 */
  sound: string;
  /** 是否触发 */
  triggered: boolean;
...
export function processEliteArmor(
  ...
  if (elite && game.battleTurn % ELITE_CONFIG.eliteArmorCycle === 0 && game.battleTurn > 0) {
    ...
    return {
      ...
      float: { text: `护甲+${armorVal}`, color: '#22d3ee', target: 'enemy', icon: ICON_SHIELD },
      sound: 'enemy_defend',
      triggered: true,
      isBossResult: false,
    };
  ...
  if (boss && game.battleTurn % ELITE_CONFIG.bossArmorCycle === 0 && game.battleTurn > 0) {
    ...
    return {
      ...
      float: { text: `护盾+${armorVal}`, color: '#67e8f9', target: 'enemy', icon: ICON_ARCANE_SHIELD },
      sound: 'enemy_defend',
      triggered: true,
      isBossResult: true,
    };
