...
export function executePriestSkill(
  ...
  if (!isInquisitor && damagedAllies.length > 0) {
    ...
    result.floats.push({ text: `+${healVal}`, color: '#10b981', target: 'enemy', icon: ICON_HEART });
    ...
    result.floats.push({ text: `护甲+${armorVal}`, color: '#22d3ee', target: 'enemy', icon: ICON_SHIELD });
  } else {
    ...
    if (isInquisitor && debuffRoll < 0.5) {
      ...
      result.floats.push({ text: '虚弱!', color: '#c084fc', target: 'player' });
      result.floats.push({ text: '易伤!', color: '#fb923c', target: 'player' });
      ...
      result.floats.push({ text: `+${curseName}`, color: '#f87171', target: 'player' });
...
export function executeCasterSkill(e: Enemy): CasterSkillResult {
  ...
  if (dotRoll < CASTER_CONFIG.poisonChance) {
    ...
    return {
      ...
      floats: [{ text: `毒素+${poisonVal}`, color: '#34d399', target: 'player' }],
    };
  }
  ...
  if (dotRoll < CASTER_CONFIG.fireballThreshold) {
    ...
    return {
      ...
      floats: [{ text: `灼烧+${burnVal}`, color: '#fb923c', target: 'player' }],
    };
  }
  ...
  return {
    ...
    floats: [
      { text: `毒素+${poisonVal}`, color: '#34d399', target: 'player' },
      { text: '虚弱', color: '#c084fc', target: 'player', delay: 200 },
    ],
  };
}
