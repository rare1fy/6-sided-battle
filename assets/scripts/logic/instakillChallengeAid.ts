...
export function triggerInstakillChallengeAid(ctx: PostPlayContext): void {
  ...
  if (aidType === 1) {
    ...
    addFloatingText(`✦ 弱点击破 ✦`, '#fde047', undefined, 'enemy', true);
    ...
    scheduleAidEffect(800, () => {
      ...
      setTimeout(() => {
        ...
        affected.forEach(({ uid, dmg }) => {
          ...
          addFloatingText(`-${dmg}`, '#ef4444', heartIcon(), 'enemy');
        });
        setScreenShake(true);
    ...
    scheduleAidEffect(800, () => {
      ...
      setTimeout(() => {
        ...
        preCalcAffected.forEach(({ uid, cut }) => {
          setEnemyEffectForUid(uid, 'hit');
          addFloatingText(`-${cut}`, '#ef4444', heartIcon(), 'enemy');
        });
        setScreenShake(true);
        setTimeout(() => setScreenShake(false), 300);
      }, 16);
    ...
    addFloatingText(`✦ 战意觉醒 ✦`, '#fde047', undefined, 'enemy', true);
    ...
    addFloatingText(`✦ 掠夺者印记 ✦`, '#fde047', undefined, 'enemy', true);
