...
export function applyDamageToEnemies(ctx: DamageAppContext): {
    ...
    if (hasAoe) {
      ...
      aliveEnemies.forEach((e, idx) => {
        setTimeout(() => {
          ...
          if (absorbed > 0) addFloatingText(`-${absorbed}`, '#60a5fa', 'shield', 'enemy');
          if (hpDamage > 0) addFloatingText(`-${hpDamage}`, '#ef4444', 'heart', 'enemy');
        }, idx * 150);
      });
    } else {
      ...
      const hpDamage = Math.max(0, outcome.damage - absorbed);
      if (absorbed > 0) addFloatingText(`-${absorbed}`, '#60a5fa', 'shield', 'enemy');
      if (hpDamage > 0) setTimeout(() => addFloatingText(`-${hpDamage}`, '#ef4444', 'heart', 'enemy'), absorbed > 0 ? 150 : 0);
    }
  }
  ...
  if (outcome.armor > 0) {
    setArmorGained(true);
    playSound('armor');
    addFloatingText(`+${outcome.armor}`, '#60a5fa', 'shield', 'player');
    setTimeout(() => setArmorGained(false), 500);
  }
  if (outcome.heal > 0) {
    setHpGained(true);
    playSound('heal');
    addFloatingText(`+${outcome.heal}`, '#10b981', 'heart', 'player');
  ...
  if (outcome.statusEffects && outcome.statusEffects.length > 0) {
    if (isElementalAoe) {
      // 高阶同元素牌型：状态效果AOE全体敌人
      addFloatingText('元素爆发!', '#fbbf24', undefined, 'enemy');
    }
    ...
    setEnemies(prev => prev.map(e => {
      ...
      if (e.combatType === 'warrior' && e.hp > 0 && e.hp < e.maxHp) {
        ...
        if (after.bloodFury && (after.bloodFury > (e.bloodFury || 0))) {
          addFloatingText(`血怒: ×${after.bloodFury}`, '#f87171', undefined, 'enemy');
        }
        return after;
      }
      return e;
    ...
    setEnemies(curr => {
      ...
      curr.forEach(before => {
        ...
        if (after && (after.vengeance || 0) > (before.vengeance || 0)) {
          addFloatingText(`复仇 ×${after.vengeance}`, '#f87171', 'zap', 'enemy', true);
          anyTriggered = true;
        }
      });
      // [VENGEANCE-FX 2026-05-10] 触发屏抖（短促），强化"队友死了，活下来的更狠"反馈
      if (anyTriggered && typeof window !== 'undefined') {
        // Vengeance triggered — UI listens via EventBus
      }
    ...
    if (hasSplinter) {
      ...
      if (splinterDmg > 0) {
        ...
        if (otherEnemies.length > 0) {
          ...
          addFloatingText(`溅射-${splinterDmg}`, '#fb923c', undefined, 'enemy');
          setEnemies(prev => prev.map(e => {
  ...
  if (comboSplashDie && (game.comboCount || 0) >= 1) {
    ...
    if (otherAlive.length > 0) {
      ...
      addFloatingText(`连锁-${splashDmg}`, '#67e8f9', undefined, 'enemy');
      setEnemies(prev => prev.map(e => {
  ...
  if (chainBoltDie) {
    ...
    if (aliveEnemies.length > 0) {
      addFloatingText(`奥术飞弹 ×${aliveEnemies.length}`, '#93c5fd', undefined, 'enemy');
      setEnemies(prev => prev.map(e => {
        if (e.hp <= 0) return e;
        const eArm = e.armor;
    ...
    if (hasSplash) {
      ...
      if (splashDie) {
        ...
        if (otherEnemies.length > 0) {
          ...
          addFloatingText(`暗影-${splashDmg}`, '#4ade80', undefined, 'enemy');
          setEnemies(prev => prev.map(e => {
            if (e.uid !== splashTarget.uid) return e;
            const sArm = e.armor;
            const sDmg = Math.max(0, splashDmg - sArm);
  ...
  if (outcome.trueDamage > 0 && targetEnemy.hp > 0) {
    ...
    addFloatingText(`真伤 ${trueDmg}`, '#fde047', undefined, targetUid);
  }
  ...
  if (outcome.guaranteedHpPercent > 0 && targetEnemy.hp > 0) {
    ...
    addFloatingText(`削血 ${guaranteedDmg}`, '#fb923c', undefined, targetUid);
  }
  ...
  if (outcome.controlType) {
    ...
    controlTargets.forEach(e => {
      ...
      if (result.success) {
        addFloatingText(`${outcome.controlType}!`, '#67e8f9', undefined, e.uid);
      } else if (result.fizzled) {
        addFloatingText('抵抗!', '#9ca3af', undefined, e.uid);
      }
    });
  ...
  if (outcome.vulnerableToRandom > 0) {
    ...
    if (aliveOthers.length > 0) {
      ...
      addFloatingText(`易伤 +${outcome.vulnerableToRandom}`, '#f472b6', undefined, randomTarget.uid);
    }
  }
  ...
  if (outcome.damagePerCleanse > 0 && targetEnemy.hp > 0) {
    ...
    addFloatingText(`净化伤害 ${outcome.damagePerCleanse}`, '#86efac', undefined, targetUid);
  }

  // === v0.5: 散打乘区（已在 outcome.damage 中计算，这里只做飘字提示） ===
  if (outcome.scatterBonusMult > 0) {
    addFloatingText(`散打 x${(1 + outcome.scatterBonusMult).toFixed(1)}`, '#fcd34d', undefined, 'player');
  }
  ...
  if (outcome.bloodChain && targetEnemy.hp > 0) {
    ...
    if (targetIdx >= 0) {
      ...
      setEnemies(chained);
      addFloatingText('血锁链!', '#dc2626', undefined, targetUid);
  ...
  if (outcome.soloSeal && targetEnemy.hp > 0) {
    ...
    if (targetIdx >= 0) {
      ...
      addFloatingText('单挑!', '#f97316', undefined, targetUid);
      playSound('solo_seal');
    }
  }
  ...
  if (outcome.berserk) {
    ...
    addFloatingText(`狂暴! ${outcome.berserk.duration}回合`, '#ef4444', undefined, 'player');
    playSound('berserk');
  }
  ...
  // 自伤飘字
  if (totalSelfDmg > 0) {
    addFloatingText(`自伤 -${totalSelfDmg}`, '#fca5a5', undefined, 'player');
  }
