...
export async function processTurnEnd(ctx: TurnEndContext): Promise<void> {
  ...
  if (game.playerClass === 'mage' && !playedThisTurn) {
    ...
    if (currentCharge >= maxChargeForHand) {
      ...
      addFloatingText(`过充! 伤害+${Math.round(((game.mageOverchargeMult || 0) + overchargeBonus) * 100)}%`, '#c084fc', undefined, 'player');
      ...
      addFloatingText(`吟唱 ${newHandLimit}/6`, '#c084fc', magicIcon(), 'player');
      addFloatingText(`奥术屏障: +${shieldGain}`, REWARD_COLOR, arcaneShieldIcon(), 'player');
      emitReward('shield', shieldGain);
    }
      ...
      if (def.onPlay?.purifyOneOnSkip) {
        setGame(prev => {
          ...
          if (negStatuses.length > 0) {
            ...
            addFloatingText(`冥想净化: ${toRemove.type}`, '#86efac', undefined, 'player');
    ...
    if (hasTaunt) {
      ...
      if (totalTauntDmg > 0) {
        setTimeout(() => {
          ...
          addFloatingText(`-${totalTauntDmg}`, '#ef4444', heartIcon(), 'player');
