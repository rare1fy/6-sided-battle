...
export function executeDrawPhase(ctx: DrawPhaseContext): void {
  ...
  if (g2.playerClass === 'mage') {
    ...
    if (hasKeepOnce && g2.playsLeft < g2.maxPlays) {
      ...
      addFloatingText('命运之轮: 保留手牌!', '#fde047', undefined, 'player');
    } else {
      discardFromHand = dice.filter(d => !d.spent).map(d => d.diceDefId);
      remainingDice = [];
    }
  ...
  if (keepHighest > 0 && discardFromHand.length > 0) {
    ...
    if (toKeep.length > 0) addFloatingText(`保留${toKeep.map(d => d.value).join(',')}点骰子`, '#67e8f9', undefined, 'player');
  }
  ...
  setTimeout(() => {
    ...
    if (turnStartArmor > 0) {
      ...
      addFloatingText(`护甲+${turnStartArmor}`, '#67e8f9', undefined, 'player');
    }
    ...
    if (g.playerClass === 'warrior' && rawTargetHandSize > 6) {
      ...
      if (overflowMult > 0) {
        setTimeout(() => addFloatingText(`过充+${Math.round(overflowMult * 100)}%`, '#ef4444', undefined, 'player'), 200);
      }
    ...
    const keptDice: Die[] = remainingDice.map((d) => {
      ...
      if (def.onPlay?.bonusOnKeep) {
        ...
        addFloatingText(`${def.name}+${def.onPlay.bonusOnKeep}点`, '#22d3ee', undefined, 'player');
      ...
      if (def.onPlay?.bonusPerTurnKept) {
        ...
        if (accumulated < cap) {
          ...
          addFloatingText(`${def.name}+${bonus}点(${accumulated + bonus}/${cap})`, '#c084fc', undefined, 'player');
          return {
            ...d,
            value: newValue,
            selected: false,
    ...
    keptDice.forEach((d, idx) => {
      ...
      if (def.onPlay?.rerollOnKeep) {
        ...
        addFloatingText(`${def.name}自动重投`, '#60a5fa', undefined, 'player');
      }
    });
    ...
    if (keepMultBonus > 0) {
      ...
      addFloatingText(`蓄力倍率+${Math.round(keepMultBonus * 100)}%`, '#c084fc', undefined, 'player');
    }
