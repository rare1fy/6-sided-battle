...
function applyDotSettlementSideEffects(logs: DotLogEntry[], cb: EnemyAICallbacks): void {
  for (const entry of logs) {
    cb.addLog(`${entry.name} 因${entry.color === '#f97316' ? '灼烧' : '中毒'}受到了 ${entry.damage} 点伤害。`);
...
export async function executeEnemyTurn(
  ...
  cb.setGame((prev: GameState) => {
    ...
    if (absorb.absorbedByShield > 0) {
      ...
      cb.addFloatingText(`-${absorb.absorbedByShield}`, '#67e8f9', ICON_ARCANE_SHIELD, 'player');
    }
    if (absorb.hpDamage > 0) {
      cb.addLog(`你因中毒受到了 ${absorb.hpDamage} 点伤害。`);
      cb.addFloatingText(`-${absorb.hpDamage}`, '#c084fc', ICON_HEART, 'player');
    }
  ...
  for (const e of currentEnemies.filter(en => en.hp > 0)) {
    ...
    if (isFrozen) {
      ...
      cb.addFloatingText('冻结', '#22d3ee', undefined, 'enemy');
    ...
    if (isStunned) {
      ...
      cb.addFloatingText('眩晕', '#facc15', undefined, 'enemy');
      cb.setEnemyEffectForUid(e.uid, 'shake');
      await new Promise(r => setTimeout(r, 400));
      cb.setEnemyEffectForUid(e.uid, null);
      continue;
    ...
    if (dispatchedType === '技能') {
      ...
      if (isMartial && handledByDispatch && (dotType || ctlType)) {
        ...
        if (dotType) {
          ...
          const color = dotType === 'burn' ? '#fb923c' : '#34d399';
          ...
          const color = ctlType === 'freeze' ? '#22d3ee' : ctlType === 'weak' ? '#c084fc' : '#fb923c';
          ...
          if (candidates.length > 0) {
            ...
            cb.addFloatingText(`护甲+${armorVal}`, '#22d3ee', ICON_SHIELD, 'enemy');
    ...
    cb.setGame((prev: GameState) => {
      ...
      if (!mainCounted) {
        if (absorb.absorbedByShield > 0) cb.addFloatingText(`-${absorb.absorbedByShield}`, '#67e8f9', ICON_ARCANE_SHIELD, 'player');
        if (absorb.absorbedByArmor > 0) cb.addFloatingText(`-${absorb.absorbedByArmor}`, '#60a5fa', ICON_SHIELD, 'player');
        if (absorb.hpDamage > 0) cb.addFloatingText(`-${absorb.hpDamage}`, '#ef4444', ICON_HEART, 'player');
        if (absorb.absorbedByShield === 0 && absorb.absorbedByArmor === 0 && absorb.hpDamage === 0) cb.addFloatingText('0', '#9ca3af', undefined, 'player');
    ...
    if (furyDice) {
      ...
      cb.addFloatingText(`怒火+1`, '#fb923c', undefined, 'player');
    }
    ...
    if (e.combatType === 'ranger') {
      ...
      cb.setGame((prev: GameState) => {
        ...
        if (absorb2.absorbedByShield > 0 && !rangerCounted) cb.addFloatingText(`-${absorb2.absorbedByShield}`, '#67e8f9', ICON_ARCANE_SHIELD, 'player');
        if (absorb2.absorbedByArmor > 0 && !rangerCounted) cb.addFloatingText(`-${absorb2.absorbedByArmor}`, '#60a5fa', ICON_SHIELD, 'player');
        if (absorb2.hpDamage > 0 && !rangerCounted) cb.addFloatingText(`-${absorb2.hpDamage}`, '#fb923c', ICON_HEART, 'player');
    ...
    if (e.combatType === 'ranger' && e.archetype === 'trapper') {
      ...
      cb.addFloatingText('毒素+1', '#34d399', undefined, 'player');
    }
    ...
    //   rider 由上方技能分支前置写入 e.__rider，这里在攻击命中后追加挂状态。
    {
      ...
      if (rider) {
        if (rider.dot) {
          ...
          const color = rider.dot === 'burn' ? '#fb923c' : '#34d399';
          ...
          const color = rider.ctl === 'freeze' ? '#22d3ee' : rider.ctl === 'weak' ? '#c084fc' : '#fb923c';
          cb.addFloatingText(`${label}!`, color, undefined, 'player');
  ...
  cb.setGame((prev: GameState) => {
    ...
    if (absorb.hpDamage > 0) {
      ...
      cb.addFloatingText(`-${absorb.hpDamage}`, '#f97316', ICON_HEART, 'player');
    }
