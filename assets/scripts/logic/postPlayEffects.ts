...
export function executePostPlayEffects(ctx: PostPlayContext): void {
  ...
  setTimeout(() => {
    ...
    if (ch && !ch.completed && ch.progress && ch.progress > 0 && ch.value && ch.value > 0) {
      addFloatingText(`◆ ${ch.progress}/${ch.value}`, '#fbbf24', undefined, 'enemy');
      playSound('coin');
      // 敌人受击反馈：轻微震动
      enemies.filter(e => e.hp > 0).forEach(e => {
            ...
            killedEnemiesData.forEach(killedData => {
              ...
              if (overkill > 0) {
                ...
                if (aliveOthers.length > 0) {
                  ...
                  addFloatingText('-' + overkill, '#fb923c', null, 'enemy');
                  playSound('hit');
                }
              }
            });
  ...
  if (game.playerClass === 'rogue' && currentComboForShadow >= 1) {
    ...
    if (hasComboPersist && tempDieToGrant) {
      ...
      addFloatingText('连击心得: 残骰持久化!', '#4ade80', undefined, 'player');
    }
  ...
  if (currentCombo >= 1 && targetUid) {
    ...
    if (hasDoublePoison) {
      setEnemies(prev => prev.map(e => {
        ...
        addFloatingText(`蚀骨连击: 毒层翻倍(${doubled})`, '#22c55e', undefined, 'enemy');
  ...
  if (hasShadowClone && outcome.damage > 0) {
    ...
    if (cloneDmg > 0) {
      setTimeout(() => {
        addFloatingText(`影分身: ${cloneDmg}伤害`, '#c084fc', undefined, 'enemy');
      }, 400);
    }
  }
  ...
  if (outcome.holyPurify) {
    ...
    if (negativeStatuses.length > 0) {
      ...
      addFloatingText(`净化 ${toPurge.length > 1 ? '×' + toPurge.length : purgedNames}`, '#67e8f9', undefined, 'player');
    } else {
      ...
      if (cursedIdx >= 0) {
        ...
        addFloatingText(`净化 ${cursedDef.name}`, '#67e8f9', undefined, 'player');
      }
    }
  }
  ...
  if (outcome.permanentFaceBonus > 0) {
    ...
    if (eligibleDice.length > 0) {
      ...
      addFloatingText(`面值+${outcome.permanentFaceBonus}!`, '#fbbf24', undefined, 'player');
    }
  }
  ...
  if (outcome.drawBonus > 0) {
    ...
    addFloatingText(`下回合+${outcome.drawBonus}抽!`, '#93c5fd', undefined, 'player');
  }
