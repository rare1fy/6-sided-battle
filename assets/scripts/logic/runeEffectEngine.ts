...
function processOneHoldEffect(
  ...
  if (hold.damageToBloodChainTargets && game.bloodChainTarget) {
    ...
    if (target) {
      ...
      result.messages.push({ text: `${defName}: 血锁伤害 ${hold.damageToBloodChainTargets}`, color: '#f87171', targetUid: target.uid });
    }
  }

  if (hold.nextScarMult) {
    result.flags.nextScarMult = hold.nextScarMult;
    result.messages.push({ text: `${defName}: 伤痕倍率 x${hold.nextScarMult}`, color: '#fb923c' });
  }
  ...
  if (hold.armorGainPercent) {
    ...
    if (hpLost > 0) {
      ...
      result.messages.push({ text: `${defName}: +${armorGain} 护甲`, color: '#60a5fa' });
    }
  }
  ...
  if (hold.boostLowestDie) {
    ...
    if (nonRune.length > 0) {
      ...
      result.messages.push({ text: `${defName}: 最低骰+${hold.boostLowestDie}伤害`, color: '#c084fc' });
    }
  }
  ...
  if (hold.transformLowestToShadow) {
    ...
    if (nonRune.length > 0) {
      ...
      result.messages.push({ text: `${defName}: 变形残骰`, color: '#9ca3af' });
    }
  }
  ...
  if (hold.autoPoison) {
    ...
    if (aliveEnemies.length > 0) {
      ...
      result.messages.push({ text: `${defName}: 自动叠毒 +${hold.autoPoison}`, color: '#4ade80', targetUid: aliveEnemies[0].uid });
    }
  }
...
export function processCastEffect(
  ...
  if (cast.consumeAllScar && cast.trueDamagePerScar) {
    ...
    if (scars > 0) {
      ...
      result.messages.push({ text: `${defName}: 消耗${scars}层伤痕 → ${totalTrueDmg}真伤`, color: '#ef4444' });
    }
  }

  if (cast.nextBasicAttackPointsDouble) {
    result.flags.nextBasicAttackPointsDouble = true;
    result.messages.push({ text: `${defName}: 下次普攻点数翻倍!`, color: '#f97316' });
  }
  ...
  if (cast.consumeAllArmor && cast.aoeDamagePercentOfArmor) {
    ...
    if (armor > 0) {
      ...
      result.messages.push({ text: `${defName}: 消耗${armor}护甲 → AOE ${aoeDmg}伤害`, color: '#3b82f6' });
    }
  }
  ...
  if (cast.chantTurnsBonus) {
    ...
    if (cast.barrierPerChantTurn) {
      ...
      result.messages.push({ text: `${defName}: 吟唱+${cast.chantTurnsBonus} → 屏障${barrier}`, color: '#a855f7' });
    }
  }
  ...
  if (cast.controlType) {
    ...
    if (aliveEnemies.length > 0) {
      ...
      result.messages.push({ text: `${defName}: ${cast.controlType}!`, color: '#06b6d4', targetUid: aliveEnemies[0].uid });
  ...
  if (cast.transformAllToShadow) {
    ...
    result.messages.push({ text: `${defName}: 全部变残骰!`, color: '#6b7280' });
  }

  if (cast.comboMultiplierDouble) {
    result.flags.comboMultiplierDouble = true;
    result.messages.push({ text: `${defName}: 连击加成翻倍!`, color: '#eab308' });
  }
  ...
  if (cast.detonateAllPoison && cast.damagePerPoisonLayer) {
    ...
    enemies.filter(e => e.hp > 0).forEach(e => {
      ...
      if (poisonStacks > 0) {
        ...
        result.messages.push({ text: `${defName}: 引爆${poisonStacks}毒 → ${detonateDmg}伤害`, color: '#22c55e', targetUid: e.uid });
      }
    });
  }
